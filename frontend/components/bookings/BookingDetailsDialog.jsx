...
export function BookingDetailsDialog({
  ...
  const [cancelReasonError, setCancelReasonError] = React.useState(null);
  ...
  const handleStatusAction = async (action) => {
    ...
    try {
      ...
      const msg = err.response?.data?.error?.message || `Failed to perform ${action} action.`;
      toast.error(msg);
    } finally {
      setActioning(false);
    }
  ...
  const handleCounterSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.error?.message || "Failed to submit counter offer.";
      toast.error(msg);
    } finally {
      setActioning(false);
    }
                  ...
                  {canReview && (
                    <div className="flex items-center justify-between p-4 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-300">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-xs">Event Completed! Leave your performance review.</span>
                      </div>
                      <Button
                        size="sm"
