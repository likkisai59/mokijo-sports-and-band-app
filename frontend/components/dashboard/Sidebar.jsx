...
export default function Sidebar() {
    ...
    return (
        <aside className="sidebar">
            ...
            {!isMember && (
                <>
                    <button 
                        ...
                        style={{ width: '100%', border: 'none', background: 'transparent', textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
                    >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <span className="icon">
                                <svg viewBox="0 0 24 24" width="22" height="22" stroke="currentColor" strokeWidth="2" fill="none"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                            </span>
                            <span>Groups</span>
                        </div>
                        <span style={{ transition: 'transform 0.2s', transform: isGroupsExpanded ? 'rotate(180deg)' : 'rotate(0deg)', display: 'inline-flex' }}>
                    ...
                    {isGroupsExpanded && (
                        <div className="sidebar-group-list" style={{ paddingLeft: '24px', display: 'flex', flexDirection: 'column', gap: '2px', margin: '4px 0 8px 0' }}>
                            {groups.length > 0 ? (
                                groups.map((group) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" strokeWidth="2" fill="none"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                                            {group.group_name}
                                        </Link>
                                    );
                                })
                            ) : (
                                <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)', padding: '10px 8px', display: 'flex', alignItems: 'center', gap: '10px', margin: 0 }}>
                                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" strokeWidth="2" fill="none"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                                    No groups
                                </p>
                            )}
            ...
            {!isMember && (
                <Link href="/dashboard/matches" className={`menu-item ${pathname.startsWith('/dashboard/matches') ? 'active' : ''}`} style={{ textDecoration: 'none' }}>
                    <span className="icon">
                        <svg viewBox="0 0 24 24" width="22" height="22" stroke="currentColor" strokeWidth="2" fill="none"><circle cx="12" cy="12" r="10"/><path d="M8 12h8"/><path d="M12 8v8"/></svg>
                    </span>
