...
export default function ArtistProfilePage() {
  ...
  const fetchProfile = React.useCallback(async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.error?.message || "Failed to load performer profile.";
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdateSuccess = async (formData) => {
    try {
      ...
      const errMsg = err.response?.data?.error?.message || "Failed to update profile settings.";
      toast.error(errMsg);
    }
  };
