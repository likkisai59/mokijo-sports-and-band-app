...
export default function VenueOverviewPage() {
    ...
    if (loading) return (
        <div className="vd-loading"><div className="vd-spinner" /> Loading overview…</div>
    );
        ...
        <>
            ...
            <div className="vd-stats-row">
                <StatCard
                    ...
                    value={venues.length}
                    label="Active Venues"
                    icon={<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>}
                />
                <StatCard
                    color="cyan"
                    value={totalBookings}
                ...
                <StatCard
                    ...
                    value={`${avgOccupancy}%`}
                    label="Avg Occupancy"
                    icon={<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>}
                />
            </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            {allPeaks.slice(0, 5).map((p, i) => (
                                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                    <span style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", width: 80, flexShrink: 0 }}>{p.time}</span>
                                    <div style={{ flex: 1, background: "rgba(255,255,255,0.06)", borderRadius: 4, height: 8, overflow: "hidden" }}>
                                        <div style={{ height: "100%", width: `${p.percentage}%`, background: "linear-gradient(90deg,#bffe00,#00f0ff)", borderRadius: 4 }} />
                                    </div>
                                    <span style={{ fontSize: 12, color: "#bffe00", width: 36, textAlign: "right" }}>{p.percentage}%</span>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            <div className="vd-card" style={{ marginTop: 0 }}>
                <div className="vd-card-header">
                    ...
                    <a href="/venue-dashboard/my-venues" className="vd-card-action">View all →</a>
                </div>
                {venues.length === 0 ? (
                    <div className="vd-empty">
                    ...
                    <table className="vd-table">
                        <thead>
                            <tr><th>Venue</th><th>Location</th><th>Sports</th><th>Slots</th><th>Rating</th></tr>
                        </thead>
                        <tbody>
                            {venues.map((v, i) => {
                                const sports = (() => { try { return JSON.parse(v.sports_supported || "[]"); } catch { return []; } })();
                                const a = analytics[i];
                                return (
                                    <tr key={v.id}>
                                        <td style={{ fontWeight: 600 }}>{v.name}</td>
                                        <td>{v.location}</td>
                                        <td>{sports.slice(0, 2).join(", ")}{sports.length > 2 ? ` +${sports.length - 2}` : ""}</td>
